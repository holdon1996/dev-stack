...
const PageTunnels = () => {
  const {
    ...
    tunnelLogs, tunnelInstalled,
    setTunnelProvider, setTunnelPort, setTunnelProtocol,
    startTunnel, stopTunnel, installTunnelBinary, clearTunnelLogs, showToast, t
  } = useStore();
      ...
      <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-5">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-1.5 mt-1">
                ...
                {tunnelInstalled[p.id] ? (
                  <span className="text-[10px] text-accent font-bold flex items-center gap-1"><Check size={10} /> Ready</span>
                ) : (
                  <span className="text-[10px] text-muted font-bold">Not installed</span>
                )}
              </div>
            ...
            {currentProvider?.needsAuth && (
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  placeholder="Enter token..." disabled={tunnelStatus !== 'stopped'}
                />
              </div>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            {!isInstalled ? (
              ...
              >
                {tunnelStatus === 'installing'
                  ...
                  : <><Download size={14} /> Install {currentProvider?.name}</>
                }
              </button>
              ...
              <button
                className="btn-primary flex items-center gap-2"
                onClick={startTunnel} disabled={isBusy}
              >
                {tunnelStatus === 'starting'
                  ? <><Loader size={14} className="animate-spin" /> Connecting...</>
                  : <><Play size={14} /> Start Tunnel</>
                }
              </button>
            )}
            ...
            <div className="flex items-center gap-2 ml-auto">
              <span className={`status-dot ${
                tunnelStatus === 'running' ? 'dot-running' :
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden flex-1 min-h-[140px] flex flex-col">
          <div className="px-4 py-2.5 border-b border-border flex items-center justify-between">
            <span className="text-[11px] font-bold text-muted tracking-[0.08em] uppercase">Output</span>
          ...
          <div className="flex-1 p-4 overflow-y-auto font-mono text-[12px] leading-[1.8] bg-[#0c0d10]">
            {tunnelLogs.length === 0 ? (
              <span className="text-muted italic">Waiting for tunnel activity...</span>
            ) : (
              tunnelLogs.map((log, i) => (
                <div key={i} className={`log-line ${
                  log.l === 'ok' ? 'log-ok' : log.l === 'warn' ? 'log-warn' : log.l === 'err' ? 'log-err' : 'log-info'
                }`}>
